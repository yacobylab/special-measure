...
switch ico(3)
    ...
    case 4
        ...
        if nrec(1) == 0
            ...
            daqfn('BeforeAsyncRead',  smdata.inst(ico(1)).data.handle, ico(2), 0, ...
                nsamp, 1, nrec(min(2, end)), 1024+1);% uses total # records                        
    ...
    case 5
        ...
        nrec = max(1, 2^(ceil(log2(val*downsamp))-21));
        if nrec > 1 || nargin >= 4
            ...
            smdata.inst(ico(1)).data.nabuffers={};
            for i=1:maxbuf % Number of buffers to use in acquisiton; 
              nsamp=val*downsamp/nrec(1);
              smdata.inst(ico(1)).data.nabuffers{i}=libpointer('uint16Ptr', zeros(nsamp+16+32, 1, 'uint16'));              
              aloff=mod(calllib('printpointer','printpointer',smdata.inst(ico(1)).data.nabuffers{i}),64);
              smdata.inst(ico(1)).data.buffers{i}=smdata.inst(ico(1)).data.nabuffers{i}+(16-(aloff/2));
            end 
