function [val, rate] = smcATS9440v1(ico, val, rate, varargin)
    ...
    case 4
        ...
        if nrec(1) == 0
            ...
            daqfn('BeforeAsyncRead',  smdata.inst(ico(1)).data.handle, ico(2), 0, ...
                nsamp, 1, nrec(min(2, end))+extracap, admaFlags); % uses total # records            
            for i=1:length(smdata.inst(ico(1)).data.buffers) % Number of buffers to use in acquisiton;
                %last arg is length of buffer in bytes, (uint16 = 2 * 8bits/byte * nsamp)  
    ...
    case 5
       ...
       daqfn('SetRecordCount', smdata.inst(ico(1)).data.handle, 1) 
        ...
        retcode = calllib('ATSApi', 'AlazarSetTriggerOperation', ...
            smdata.inst(ico(1)).data.handle,		...	% HANDLE -- board handle
            0,	...	% U32 -- trigger operation
            0,		...	% U32 -- trigger engine id
            2,		...	% U32 -- trigger source id
            2,	... % U32 -- trigger slope id
            128+28,				...	% U32 -- trigger level from 0 (-range) to 255 (+range)
            1,		...	% U32 -- trigger engine id
            3,		...	% U32 -- trigger source id for engine K
            1, ...	% U32 -- trigger slope id
            128					...	% U32 -- trigger level from 0 (-range) to 255 (+range)
            );
        if retcode ~=512
           error('alazar error setting trigger parameters') 
        end
