...
/* Writes an unsigned 32-bit noncomplex scalar to control. */

void lia_writeU32(double control[], uint32_t value[])
{
	...
	NiFpga_WriteU32(session, control[0], value[0]);
                  ...
                  int nrhs, const mxArray *prhs[] )
{
  ...
  /* The value input must be a noncomplex scalar unsigned int32.*/
  mrows = mxGetM(prhs[0]);
  ncols = mxGetN(prhs[0]);
  ...
  if( !mxIsUint32(prhs[1]) || mxIsComplex(prhs[1]) ||
      !(mrows==1 && ncols==1) ) {
    mexErrMsgTxt("Write value input must be a noncomplex scalar unsigned int32.");
  }
