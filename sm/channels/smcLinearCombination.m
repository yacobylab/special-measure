...
% allows a linear combination of two channels to be controlled in a consistent manner.
% function val = smcLinearCombination(ic, val, rate)
%
...
switch ic(3) % OPERATION TO PERFORM
    case 1 % SET
        ...
        %Calculate the maximum change in physical channel value and the corresponding index
        [~, maxindex]=max(abs(deltaphysicalvals));
        %Set the ramp rate according to actual required change in physical channel value:
